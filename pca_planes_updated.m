function [dip,strike,L,H,p,xp,yp,zp] = pca_planes_updated(c11)
...
%
%This updated version of the function benefits from the contribution of 
%Cliff Thurber, who modified the representation of planes so that their 
%dimensions are not limited to the approximation of Ouillon et al. (2008).
%
...
%% The following lines of code (55-60) are provided by Cliff Thurber, 
% to whom I extend my sincere thanks
% c11(:,1) c11(:,2) c11(:,3)
B1 = [c11(:,1), c11(:,2), ones(size(c11,1),1)] \ c11(:,3);
xv = [min(c11(:,1)), max(c11(:,1))]';
yv = [min(c11(:,2)), max(c11(:,2))]';
[X,Y] = meshgrid(xv, yv);
zp = reshape([X(:), Y(:), ones(size(X(:)))] * B1, numel(xv), []);
